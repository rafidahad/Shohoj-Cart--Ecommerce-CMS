  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-lime-50 flex flex-col">
      ...
      <div className="flex flex-1">
        {/* Sidebar */}
        <aside className="w-64 bg-white/90 border-r min-h-full py-8 px-3 flex flex-col gap-3 shadow-lg">
        ...
        <main className="flex-1 p-10">
          <h1 className="text-3xl font-extrabold mb-6 text-gray-800">Customers</h1>
          <div className="bg-white rounded-2xl p-8 shadow-xl border border-green-100">
            <p className="text-gray-600 text-lg">This is the Customers page. List and manage your customers here.</p>
          </div>
        </main>
      </div>
    </div>
...
function SidebarLink({ label, color, path }) {
  ...
  return (
    <div
      ...
      style={{ pointerEvents: isActive ? 'none' : 'auto', opacity: isActive ? 1 : undefined }}
    >
      {label}
    </div>
  );
